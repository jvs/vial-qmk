...
void render_large_vertical_text(const char* text) {
    oled_clear();
    for (int i = 0; text[i] != '\0'; i++) {
        oled_set_cursor(0, i * 2);
        oled_write_char(text[i], false);
        // Make it bigger by writing to adjacent positions
        oled_set_cursor(1, i * 2);
        oled_write_char(text[i], false);
        oled_set_cursor(0, i * 2 + 1);
        oled_write_char(text[i], false);
        oled_set_cursor(1, i * 2 + 1);
        oled_write_char(text[i], false);
    }
}
...
void render_left_display(void) {
    ...
    switch (current_layer) {
        ...
        case _NUMBER:
            ...
            break;
        case _VIM:
            render_large_vertical_text("VIM");
            break;
        default:
            render_large_letter(large_M);
            break;
...
void render_right_display(void) {
    ...
    if (current_layer == _VIM) {
        ...
        switch (current_vim_mode) {
            case VIM_NORMAL:
                render_large_vertical_text("N");
                break;
            case VIM_VISUAL:
                render_large_vertical_text("V");
                break;
        }
    } else {
        // Show JVS
        render_large_vertical_text("JVS");
    }
}
