...
void render_stacked_letters(const char* const letters[], int count) {
    // Each letter is 16 pixels tall (2 OLED rows)
    // OLED is 128 pixels tall, so we have room for 8 letters
    for (int i = 0; i < count && i < 8; i++) {
        // Set position for each letter - each letter takes 2 rows (16 pixels)
        oled_set_cursor(0, i * 2);
        oled_write_raw_P(letters[i], 64);
    }
}
...
void render_left_display(void) {
    ...
    if (current_layer != last_left_layer) {
        ...
        switch (current_layer) {
            ...
            case _NUMBER:
                {
                    const char* const letters[] = {large_N, large_U, large_M, large_B, large_E, large_R};
                    render_stacked_letters(letters, 6);
                }
                break;
            case _VIM:
                {
            ...
            case _SYMBOL:
                {
                    const char* const letters[] = {large_S, large_Y, large_M, large_B, large_O, large_L};
                    render_stacked_letters(letters, 6);
                }
                break;
            default:
